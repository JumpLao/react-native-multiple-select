...
export default class MultiSelect extends Component {
  ...
  _getSelectLabel = () => {
    const { selectText, single, selectedItems, displayKey, items, uniqueKey } = this.props;
    ...
    return `${selectedItems.map(k => {
      const founded = items.find(i => i[uniqueKey] == k);
      return founded ? founded[displayKey] : '';
    })}`;
  };
